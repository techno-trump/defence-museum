...
(function(global, factory) {
    ...
    var handleScroll = function handleScroll(event, targetElement) {
        if (targetElement) {
            ...
            if (!isInputControl) if (isVertical && (isOnTop || isOnBottom) || !isVertical && (isOnLeft || isOnRight)) return preventDefault(event);
        }
    ...
    var lock = function lock(targetElement) {
        ...
        } else if (lockedNum <= 0) unLockCallback = detectOS().android ? setOverflowHiddenMobile() : setOverflowHiddenPc();
        lockedNum += 1;
    };
